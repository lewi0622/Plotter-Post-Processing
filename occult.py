...
def main(input_files=()):
    ...
    if len(occlusion_file_list) == 0:
        ...
        for index, file in enumerate(occlusion_file_list):
            ...
            ttk.Checkbutton(window, text="Crop", variable=occult_crop).grid(
                sticky="e", row=current_row, column=0)
            occult_color = IntVar(window, value=0)
            occult_file_info["color"] = occult_color
            ttk.Radiobutton(window, text="Use File Color", variable=occult_color, value=0).grid(
                row=current_row, column=1)
            ttk.Radiobutton(window, text="Overwrite Color", variable=occult_color, value=1).grid(
            ...
            occult_file_info["across"] = occult_across
            ttk.Checkbutton(window, text="Occult across layers,\nnot within",
                            variable=occult_across).grid(sticky="w", row=current_row, column=3)
            current_row += 1
